...
import {
  Calendar, Video, FileText, ArrowRight, Clock, Plus,
  ChevronRight, Activity, Heart, Thermometer, Search, Loader2, MapPin, Stethoscope, ChevronDown,
} from "lucide-react";
    ...
    <div className="bg-white p-6 sm:p-8 rounded-[32px] border border-slate-100 shadow-sm">
      <div className="flex flex-col lg:flex-row lg:items-end gap-4 lg:gap-6">
        <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label htmlFor="dashboard-state" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
          ...
          <div>
            <label htmlFor="dashboard-city" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
              City
            </label>
            <div className="relative">
              <select
          ...
          <div>
            <label htmlFor="dashboard-specialty" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
      ...
      <p className="text-sm text-slate-500 mt-4">
        Book verified doctors by location and specialty. Confirmed appointments.
      </p>
    </div>
  );
}
...
export default function PatientDashboard() {
  ...
  const nextApptLabel = nextAppt
    ? format(new Date(nextAppt.scheduledAt), "MMM d")
    : "None";
  ...
  return (
    ...
    >
      ...
      {/* Find Doctors (Practo-style) */}
      <div>
        <h3 className="text-xl font-bold text-slate-900 mb-4">Find doctors near you</h3>
        <FindDoctorsSection />
      </div>

      {/* Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard icon={Heart} label="Heart Rate" value="72 bpm" color="bg-red-50 text-red-500" sub="+2% normal" />
        <StatCard icon={Activity} label="Blood Pressure" value="120/80" color="bg-blue-50 text-blue-500" sub="Stable" />
        <StatCard icon={Thermometer} label="Temperature" value="36.6 °C" color="bg-orange-50 text-orange-500" />
        <StatCard
          icon={Calendar}
      ...
      <div className="grid lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {isLoading ? (
            ...
            <div className="p-10 bg-white rounded-[32px] border border-dashed border-slate-200 text-center">
              <p className="text-slate-400 font-medium mb-4">No upcoming appointments.</p>
              <Link
                href="/patient/doctors"
          ...
          <div className="pt-4">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              >
                ...
                <p className="text-sm text-slate-500">Search for top-rated doctors.</p>
              </Link>
              <Link
        ...
        <div className="space-y-8">
          <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm">
            <h3 className="text-xl font-bold text-slate-900 mb-6">Past Consultations</h3>
            {allAppointments.filter((a) => a.status === "COMPLETED").length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-4">No past consultations.</p>
            ) : (
