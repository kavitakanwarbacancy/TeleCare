...
import { Stethoscope, Shield, Clock, Video, ArrowRight, Star, Users, Calendar, FileText } from 'lucide-react';
    ...
    <div className="min-h-screen bg-white overflow-hidden">
      ...
      <nav className="max-w-7xl mx-auto px-6 h-24 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-10">
          <a href="#features" className="text-slate-600 font-medium hover:text-brand-500 transition-colors">Features</a>
          <a href="#how-it-works" className="text-slate-600 font-medium hover:text-brand-500 transition-colors">How it works</a>
          <a href="#doctors" className="text-slate-600 font-medium hover:text-brand-500 transition-colors">For Doctors</a>
        </div>
        <div className="flex items-center gap-4">
          <Link href="/login" className="px-6 py-3 text-slate-600 font-semibold hover:text-brand-500 transition-colors">Login</Link>
          <Link href="/signup" className="px-8 py-3 bg-brand-500 text-white font-bold rounded-full shadow-lg shadow-brand-100 hover:bg-brand-600 hover:shadow-brand-200 transition-all active:scale-95">Get Started</Link>
        </div>
      </nav>
        ...
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <h1 className="text-6xl lg:text-7xl font-bold text-slate-900 leading-[1.1] mb-8 tracking-tight">
              Quality Healthcare, <span className="text-brand-500">Anywhere, Anytime.</span>
            </h1>
            <p className="text-xl text-slate-500 mb-12 leading-relaxed max-w-lg">
              Connect with top-rated doctors for instant video consultations, digital prescriptions, and personalized care from the comfort of your home.
            </p>
            <div className="flex flex-col sm:flex-row gap-4">
              <Link href="/signup" className="px-10 py-5 bg-brand-500 text-white text-lg font-bold rounded-full shadow-xl shadow-brand-100 hover:bg-brand-600 hover:shadow-brand-200 transition-all flex items-center justify-center gap-2 group">
                Book an Appointment <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </Link>
              <Link href="/patient/doctors" className="px-10 py-5 bg-white text-slate-900 border-2 border-slate-100 text-lg font-bold rounded-full hover:bg-slate-50 transition-all flex items-center justify-center gap-2">
                Find a Doctor
              </Link>
            </div>
          </motion.div>
      ...
      <section id="features" className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-3xl mx-auto mb-20">
            <h2 className="text-4xl lg:text-5xl font-bold text-slate-900 mb-6">Why Choose TeleCare?</h2>
            <p className="text-xl text-slate-500 leading-relaxed">We combine cutting-edge technology with compassionate care to provide the best healthcare experience.</p>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="bg-brand-600 rounded-[48px] p-12 lg:p-24 text-center relative overflow-hidden shadow-2xl shadow-brand-200">
            <div className="relative z-10">
              <h2 className="text-4xl lg:text-6xl font-bold text-white mb-8">Ready to start your journey?</h2>
              <p className="text-xl text-brand-100 mb-12 max-w-2xl mx-auto">Join thousands of patients who trust TeleCare for their daily healthcare needs.</p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link href="/signup" className="px-12 py-5 bg-white text-brand-600 text-lg font-bold rounded-full hover:bg-brand-50 transition-all shadow-xl">
                  Create an Account
                </Link>
                <Link href="/doctor/dashboard" className="px-12 py-5 bg-brand-700 text-white text-lg font-bold rounded-full hover:bg-brand-800 transition-all border border-brand-500">
                  Join as a Doctor
                </Link>
              </div>
            </div>
      ...
      <footer className="py-12 border-t border-slate-100">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <p className="text-slate-500 text-sm">© 2026 TeleCare Inc. All rights reserved.</p>
          <div className="flex gap-8">
            <a href="#" className="text-slate-400 hover:text-brand-500 transition-colors">Privacy Policy</a>
            <a href="#" className="text-slate-400 hover:text-brand-500 transition-colors">Terms of Service</a>
          </div>
        </div>
      </footer>
    </div>
